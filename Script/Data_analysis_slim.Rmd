...
Furthermore, it includes basic principle component analysis *(PCA)* clustering, differential expression analysis and Gene Set Enrichment Analysis *(GSEA)*.
...
#If you have troubles with the filepath, thoug the file is in the correct location, go to Tools > Global Options > R Markdown > and set evaluate chunks in directory to "Project".
...
p.GSEA <- 0.1 #p-value threshold for GSEA results
...
gseBP <- gseGO(geneList = gene_list, #gene_list contains genes and their log2FC values, which are used for ranking the genes in GSEA
             ...
             pvalueCutoff = p.GSEA, #p-value threshold for significant enrichment !!!!!!!DEFAULT: 0.10, which is rather relaxed. Adjust as desired!!!!!!
...
ggplot(topBP, aes(x = reorder(Description, NES), y = NES, color = p.adjust)) + #define the plot aesthetics, reorder the x-axis by NES and color by adjusted p-value
  ...
  annotate("text", nrow(topBP)+.4, y = max(topBP$NES)/3, label = act_lab, col = "grey10", size = 3) + #add annotation for the positive enrichment direction
  annotate("text", nrow(topBP)+.4, y = min(topBP$NES)/3, label = sup_lab, col = "grey10", size = 3) #add annotation for the negative enrichment direction
```
...
ggplot(topMF, aes(x = reorder(Description, NES), y = NES, color = p.adjust)) + #define the plot aesthetics, reorder the x-axis by NES and color by adjusted p-value
  ...
  annotate("text", nrow(topMF)+.4, y = max(topMF$NES)/3, label = act_lab, col = "grey10", size = 3) + #add annotation for the positive enrichment direction
  annotate("text", nrow(topMF)+.4, y = min(topMF$NES)/3, label = sup_lab, col = "grey10", size = 3) #add annotation for the negative enrichment direction
...
ggplot(topCC, aes(x = reorder(Description, NES), y = NES, color = p.adjust)) + #define the plot aesthetics, reorder the x-axis by NES and color by adjusted p-value
  ...
  annotate("text", nrow(topCC)+.4, y = max(topCC$NES)/3, label = act_lab, col = "grey10", size = 3) + #add annotation for the positive enrichment direction
  annotate("text", nrow(topCC)+.4, y = min(topCC$NES)/3, label = sup_lab, col = "grey10", size = 3) #add annotation for the negative enrichment direction
```
